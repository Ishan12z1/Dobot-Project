...
def load():
    ...
    if platform.system() == "Windows":
        ...
        dll_path = os.path.join(os.path.dirname(__file__), "libDobotDll.dylib")
        return CDLL(dll_path)
    else:
        dll_path = os.path.join(os.path.dirname(__file__), "libDobotDll.so")
...
def ClearAllAlarmsState(api, max_retries=50, verbose=True):
    tries = 0
    while True:
        result = api.ClearAllAlarmsState()
        if result == DobotCommunicate.DobotCommunicate_NoError:
            if verbose:
                print("[SUCCESS] Cleared alarms")
            break
        elif result == DobotCommunicate.DobotCommunicate_BufferFull:
            if verbose:
                print(f"[WARN] Buffer full at try {tries}")
        elif result == DobotCommunicate.DobotCommunicate_Timeout:
            if verbose:
                print(f"[ERROR] Timeout at try {tries}")
        else:
            if verbose:
        ...
        if max_retries and tries >= max_retries:
            raise TimeoutError(f"ClearAllAlarmsState failed after {tries} attempts (last={result})")
...
def SetHOMEParams(api,  x,  y,  z,  r,  isQueued=0):
    ...
    return [queuedCmdIndex.value]
def reset_queue(api):
    # Safely flush any backlog so new commands can be accepted
    SetQueuedCmdStopExec(api)
    SetQueuedCmdClear(api)
    SetQueuedCmdStartExec(api)

def SetHOMEParams_robust(api, x, y, z, r, isQueued=0, max_retries=10, sleep_ms=100):
    """
    Prefer isQueued=0 for parameter setters.
    If BufferFull persists, reset queue once and retry.
    """
    param = HOMEParams()
    param.x, param.y, param.z, param.r = x, y, z, r
    queuedCmdIndex = c_uint64(0)

    # 1) Try NON-queued first (recommended)
    result = api.SetHOMEParams(byref(param), 0, byref(queuedCmdIndex))
    if result == DobotCommunicate.DobotCommunicate_NoError:
        print("[SUCCESS] SetHOMEParams (non-queued)")
        return True, queuedCmdIndex.value, result

    print(f"[WARN] Non-queued SetHOMEParams failed err={result}. Will try queued path…")

    # 2) If you MUST queue, ensure queue is running & has space
    reset_queue(api)

    last_err = result
    for attempt in range(1, max_retries + 1):
        result = api.SetHOMEParams(byref(param), 1, byref(queuedCmdIndex))
        if result == DobotCommunicate.DobotCommunicate_NoError:
            print(f"[SUCCESS] SetHOMEParams (queued) on attempt {attempt}, idx={queuedCmdIndex.value}")
            return True, queuedCmdIndex.value, result

        print(f"[WARN] Attempt {attempt}/{max_retries} failed, err={result}")
        if result == DobotCommunicate.DobotCommunicate_BufferFull:
            # queue still jammed; back off
            dSleep(sleep_ms)
            if attempt == 3:  # one hard reset early if jam persists
                print("[INFO] Queue still full; resetting queue…")
                reset_queue(api)
        else:
            # For other errors (timeout/port), don’t spin pointlessly
            break
        last_err = result

    print(f"[ERROR] SetHOMEParams FAILED after {max_retries} attempts (last err={last_err})")
...
#     while(True):
#         result = api.SetIOMultiplexing(byref(param), 0, byref(queuedCmdIndex))
